...
typedef struct 
{
    ngx_str_t                  name;   		//目录的绝对路径名
    size_t                     len;			//level指定的该目录下所有子目录的总共所占用路径的字符长度(包含路径间的'/'字符)
    size_t                     level[3];	///该目录下每一级子目录的目录名占用字符的个数

    ngx_path_manager_pt        manager;		//cache manager进程针对该目录执行的函数
    ngx_path_loader_pt         loader;		//cache loader进程针对该目录执行的函数
    void                      *data;		//manager和loader函数的参数

    u_char                    *conf_file;	//指定该目录的配置文件名
    ngx_uint_t                 line;		//指定该目录的配置文件的行号
} ngx_path_t;
