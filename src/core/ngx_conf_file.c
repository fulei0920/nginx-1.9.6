...
{
	//Óï·¨: include /path/file
	//½«ÆäËûÅäÖÃÎÄ¼şÇ¶Èëµ½µ±Ç°µÄnginx.confÎÄ¼şÖĞ£¬²ÎÊı¿ÉÒÔÊÇ¾ø¶ÔÂ·¾¶£¬
	//Ïà¶ÔÂ·¾¶(Ïà¶ÔÓÚNginxµÄÅäÖÃÄ¿Â¼£¬¼´nginx.confËùÔÚµÄÄ¿Â¼)£¬
	//Ò²¿ÉÒÔÊÇ°üº¬Í¨Åä·û*µÄÎÄ¼şÃû£¬Í¬Ê±¿ÉÒÔÒ»´ÎÇ¶Èë¶à¸öÅäÖÃÎÄ¼ş
    { 
		ngx_string("include"),
...
//ÔÚÖ´ĞĞconfigureÃüÁîÊ±£¬ÎÒÃÇÒÑ¾­°ÑĞí¶àÄ£¿é±àÒë½øNginxÖĞ£¬µ«ÊÇ·ñÆôÓÃÕâĞ©Ä£¿é£¬Ò»°ãÈ¡¾öÓÚÅäÖÃÎÄ¼şÖĞÏàÓ¦µÄÅäÖÃÏî¡£
//»»¾ä»°Ëµ£¬Ã¿¸öNginxÄ£¿é¶¼ÓĞ×Ô¼º¸ĞĞËÈ¤µÄÅäÖÃÏî£¬´ó²¿·ÖÄ£¿é¶¼±ØĞëÔÚnginx.confÖĞ¶ÁÈ¡Ä³¸öÅäÖÃºó²Å»áÔÚÔËĞĞÊ±ÆôÓÃ¡£
//ÀıÈç£¬Ö»ÓĞµ±ÅäÖÃhttp{...}Õâ¸öÅäÖÃÏîÊ±£¬ngx_http_moduleÄ£¿é²Å»áÔÚNginxÖĞÆôÓÃ£¬ÆäËûÒÀÀµngx_http_moduleµÄÄ£¿éÒ²
//²ÅÄÜÕı³£Ê¹ÓÃ
...
//filename²»Îª¿Õ, ¿ªÊ¼½âÎöÒ»¸öÅäÖÃÎÄ¼ş
//filenameÎª¿Õ && cf->conf_file->file.fd != NGX_INVALID_FILE, ¿ªÊ¼½âÎöÒ»¸öÅäÖÃÎÄ¼şÖĞµÄÄ³¸öÅäÖÃ¿é
//ÆäËû(filenameÎª¿Õ && cf->conf_file->file.fd == NGX_INVALID_FILE), ½âÎöÒ»¸öÃüÁîĞĞ²ÎÊıÅäÖÃÏîÖµ
char *
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    ngx_conf_dump_t  *cd;
    enum  {
        parse_file = 0,		/*·ÖÎöÄ³¸öÎÄ¼ş*/  //½âÎöÅäÖÃÎÄ¼ş
        parse_block,		/*·ÖÎöÎÄ¼şÖĞÄ³¸ö¿éÅäÖÃÏî*/ //½âÎö¿éÅäÖÃ¡£¿éÅäÖÃÒ»¶¨ÊÇÓÉ¡°{¡±ºÍ¡°}¡±°ü¹üÆğÀ´µÄ
        parse_param   		/*·ÖÎöÆô¶¯-g²ÎÊı*/	//½âÎöÃüÁîĞĞÅäÖÃ¡£ÃüÁîĞĞÅäÖÃÖĞ²»Ö§³Ö¿éÖ¸Áî
    } type;
...
#endif
	/***********ÅĞ¶Ïµ±Ç°½âÎö¹ı³Ì×´Ì¬**********/
    if (filename) {   //ÕıÒª¿ªÊ¼½âÎöÒ»¸öÅäÖÃÎÄ¼ş
		...
		//½«µ±Ç°×¼±¸½âÎöµÄÎÄ¼şdumpµ½cycle->config_dumpÖĞ
        if (ngx_dump_config
#if (NGX_DEBUG)
            || 1
#endif
    ...
    } else if (cf->conf_file->file.fd != NGX_INVALID_FILE) { //ÕıÒª¿ªÊ¼½âÎöÒ»¸ö¸´ÔÓÅäÖÃÏîÖµ
	
        type = parse_block;
    } else {	//ÕıÒª½âÎöÃüÁîĞĞ²ÎÊıÅäÖÃÏîÖµ
...
ngx_conf_handler(ngx_conf_t *cf, ngx_int_t last)
{
    ...
    for (i = 0; ngx_modules[i]; i++) {
        ...
        for ( /* void */ ; cmd->name.len; cmd++) {
			...
			/*Ä£¿éÀàĞÍ²»ÊÇNGX_CONF_MODULEÀàĞÍ»òÕßÓëxxxÀàĞÍ²»Í¬ÔòÌø¹ı*/
			//ÅĞ¶ÏÆ¥ÅäµÄÖ¸Áî¶ÔÓ¦µÄÄ£¿éµÄÄ£¿éÀàĞÍÊÇ·ñÓëµ±Ç°½âÎöÖ¸ÁîËùÊôµÄÄ£¿éÀàĞÍÏàÍ¬
            if (ngx_modules[i]->type != NGX_CONF_MODULE && ngx_modules[i]->type != cf->module_type) {
                continue;
            }

            /* is the directive's location right ? */
            if (!(cmd->type & cf->cmd_type)) { //ÅĞ¶Ïµ±Ç°½âÎöµ½Ö¸ÁîµÄÎ»ÖÃÊÇ·ñÂú×ãÆ¥ÅäµÄÖ¸ÁîËùÒªÇóµÄÎ»ÖÃ(NGX_MAIN_CONF|NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONFµÈ)
                continue;
            }
			...
			//ÅĞ¶ÏÆ¥ÅäµÄÖ¸ÁîĞèÒªµÄ²ÎÊı¸öÊıÓë½âÎöµÄÖ¸ÁîµÄ²ÎÊı¸öÊıÊÇ·ñÏà·û
...
ngx_conf_read_token(ngx_conf_t *cf)
{
    ...
    quoted = 0;
    s_quoted = 0;
    d_quoted = 0;	//µ±Ç°´¦ÓÚË«ÒıºÅ×Ö·û´®ºó

    cf->args->nelts = 0;
    b = cf->conf_file->buffer;
    dump = cf->conf_file->dump;
    start = b->pos;							//tokenµÄÆğÊ¼µØÖ·
    start_line = cf->conf_file->line;		//tokenµÄĞĞºÅ

    file_size = ngx_file_size(&cf->conf_file->file.info);

    for ( ;; ) {
		//ÅĞ¶ÏbufferÖĞÊÇ·ñ»¹ÓĞÊı¾İÃ»ÓĞ´¦Àí
        if (b->pos >= b->last) {
			//ÅĞ¶ÏÊÇ·ñÒÑ¾­¶Áµ½ÎÄ¼şÄ©Î²
            ...
            if (len == NGX_CONF_BUFFER) {	//Èç¹ûbufferÖĞÒÑ´¦Àíµ«ÊÇ²»ÊÇÒ»¸öÍêÕûµÄtokenµÄ³¤¶ÈÕ¼¾İÁËÕû¸öbuffer´óĞ¡£¬Ôò´íÎó
            ...
            if (len) {		//½«ÒÔ´¦Àíµ«²»ÍêÕûµÄtokenÒÆµ½bufferÆğÊ¼Î»ÖÃ
                ngx_memmove(b->start, start, len);
            }

			//¼ÆËã¿ÉÒÔ´ÓÎÄ¼şÖĞ¶Áµ½bufferµÄ´óĞ¡
            size = (ssize_t) (file_size - cf->conf_file->file.offset);  //¼ÆËãÎÄ¼ş»¹Î´¶ÁÈ¡µÄ×Ö½ÚÊı

            if (size > b->end - (b->start + len)) {  //»ñÈ¡ÎÄ¼şÎ´¶ÁÈ¡µÄ×Ö½ÚÊıÓëbufferÊ£Óà¿Õ¼äµÄ½ÏĞ¡Öµ
                size = b->end - (b->start + len);
            }

            n = ngx_read_file(&cf->conf_file->file, b->start + len, size, cf->conf_file->file.offset); //´ÓÎÄ¼şÖĞ¶ÁÈ¡Êı¾İµ½bufferÖĞ
            ...
            b->pos = b->start + len; //¸üĞÂbufferÖĞµÄÏà¹Ø±êÊ¶
            b->last = b->pos + n;
            start = b->start;		//ÖØĞÂÉèÖÃtokenµÄÆğÊ¼Î»ÖÃÎªbufferµÄÆğÊ¼Î»ÖÃ
        ...
        if (last_space) {
            ...
            start = b->pos - 1;   				//¼ÇÂ¼tokenµÄÆğÊ¼Î»ÖÃ
            start_line = cf->conf_file->line; 	//¼ÇÂ¼tokenµÄĞĞºÅ
            ...
            if (found) {   //·¢ÏÖÒ»¸ötoken£¬½«Æä´æ´¢µ½cf->argsÊı×éÖĞ
                word = ngx_array_push(cf->args);  //´Ócf->argsÖĞ»ñÈ¡Ò»¸öÔªËØ
                if (word == NULL) {
                    return NGX_ERROR;
                }
                ...
                for (dst = word->data, src = start, len = 0; src < b->pos - 1; len++) {  //¿½±´tokenÍ¬Ê±´¦ÀítokenÖĞµÄ×ªÒå×Ö·û
                    if (*src == '\\') {   
                        switch (src[1]) {
                        case '"':
                        case '\'':
...
conf_prefix -- ²¼¶ûÖµ£¬true±íÊ¾ÒÔcycle->conf_prefix×÷ÎªÇ°×º£¬false±íÊ¾ÒÔcycle->prefix×÷ÎªÇ°×º
*/
ngx_int_t
