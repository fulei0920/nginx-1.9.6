...
static ngx_command_t  ngx_conf_commands[] = 
{
	...
	//½«ÆäËûÅäÖÃÎÄ¼þÇ¶Èëµ½µ±Ç°µÄnginx.confÎÄ¼þÖÐ£¬²ÎÊý¿ÉÒÔÊÇ¾ø¶ÔÂ·¾¶£¬
	//Ïà¶ÔÂ·¾¶(Ïà¶ÔÓÚNginxµÄÅäÖÃÄ¿Â¼£¬¼´nginx.confËùÔÚµÄÄ¿Â¼)£¬
	//Ò²¿ÉÒÔÊÇ°üº¬Í¨Åä·û*µÄÎÄ¼þÃû£¬Í¬Ê±¿ÉÒÔÒ»´ÎÇ¶Èë¶à¸öÅäÖÃÎÄ¼þ
    { 
		ngx_string("include"),
...
//ÔÚÖ´ÐÐconfigureÃüÁîÊ±£¬ÎÒÃÇÒÑ¾­°ÑÐí¶àÄ£¿é±àÒë½øNginxÖÐ£¬µ«ÊÇ·ñÆôÓÃÕâÐ©Ä£¿é£¬Ò»°ãÈ¡¾öÓÚÅäÖÃÎÄ¼þÖÐÏàÓ¦µÄÅäÖÃÏî¡£
//»»¾ä»°Ëµ£¬Ã¿¸öNginxÄ£¿é¶¼ÓÐ×Ô¼º¸ÐÐËÈ¤µÄÅäÖÃÏî£¬´ó²¿·ÖÄ£¿é¶¼±ØÐëÔÚnginx.confÖÐ¶ÁÈ¡Ä³¸öÅäÖÃºó²Å»áÔÚÔËÐÐÊ±ÆôÓÃ¡£
//ÀýÈç£¬Ö»ÓÐµ±ÅäÖÃhttp{...}Õâ¸öÅäÖÃÏîÊ±£¬ngx_http_moduleÄ£¿é²Å»áÔÚNginxÖÐÆôÓÃ£¬ÆäËûÒÀÀµngx_http_moduleµÄÄ£¿éÒ²
//²ÅÄÜÕý³£Ê¹ÓÃ
...
ngx_conf_read_token(ngx_conf_t *cf)
{
    ...
    quoted = 0;
    s_quoted = 0;
    d_quoted = 0;	//µ±Ç°´¦ÓÚË«ÒýºÅ×Ö·û´®ºó

    cf->args->nelts = 0;
    b = cf->conf_file->buffer;
    dump = cf->conf_file->dump;
    start = b->pos;							//tokenµÄÆðÊ¼µØÖ·
    start_line = cf->conf_file->line;		//tokenµÄÐÐºÅ
