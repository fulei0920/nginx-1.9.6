...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) 
	{
		...
		//表示目前接收到的字符流不足以构成完整的请求行，还需要更多的字符流
		//检查header_in缓冲区是否还有空闲的内存，如果还有未使用的内存可以继续接收字符流，则调用
		//ngx_http_read_request_header方法检查缓冲区是否有未解析的字符流，否则调用
		//ngx_http_alloc_large_header_buffer方法分配更大的接收缓冲区
    ...
    for ( ;; ) 
	{
        ...
        if (rc == NGX_AGAIN) 
		{
            ...
            if (r->header_in->pos == r->header_in->end) 
			{
                ...
                if (rv == NGX_DECLINED) 
				{
					//向用户返回错误并且同时退出，错误码由宏NGX_HTTP_REQUEST_HEADER_TOO_LARGE表示
        ...
        if (rc == NGX_OK) 
		{
			//将解析出的HTTP头部设置到表示ngx_http_request_t结构体header_in成员的headers链表中
        ...
        if (rc == NGX_HTTP_PARSE_HEADER_DONE)
		{
			...
			//调用方法开始使用各HTTP模块正式的在业务上处理HTTP请求
            ngx_http_process_request(r);
		...
		//向客户端发送NGX_HTTP_BAD_REQUEST宏对应的400错误码响应
...
*/
//接收客户端发来的字符流，即把内核套接字缓冲区上的字符流接收到header_in缓冲区中。
static ssize_t
ngx_http_read_request_header(ngx_http_request_t *r)
{
	...
	//没有接收到TCP流，需要继续检测这个读事件
    if (n == NGX_AGAIN)  
	{
		//检查这个读事件是否在定时器中，如果不在则调用ngx_add_timer方法向定时器添加这个读事件
        if (!rev->timer_set) 
		{
            cscf = ngx_http_get_module_srv_conf(r, ngx_http_core_module);
...
{
	//根据HTTP头部中的host字段情况，设置对应的虚拟主机配置块，
	//会导致结构体里的srv_conf、loc_conf成员被重新设置，以指向正确的虚拟主机
    if (r->headers_in.server.len == 0 && ngx_http_set_virtual_server(r, &r->headers_in.server) == NGX_ERROR)
    {
        return NGX_ERROR;
    }
	...
	//如果HTTP版本为1.1，则host头部不可以为空，否则返回400错误响应给客户端
	...
	//如果传递了Content-Length头部，那么它必须是合法的数字，否则会返回400错误响应给客户端
...
ngx_http_process_request(ngx_http_request_t *r)
{
	...
	//由于现在已经开始准备调用各HTTP模块处理请求了，因此不再存在接收HTTP请求头部超时的问题，
	//那就需要从定时器中把当前连接的读事件移除了
    if (c->read->timer_set) 
	{
	...
	//重新设置当前连接读/写事件的回调方法为ngx_http_request_handler
    c->read->handler = ngx_http_request_handler;
    c->write->handler = ngx_http_request_handler;
	//当再次有读事件到来时，将会调用read_event_handler方法处理请求。而这里将它设置为ngx_http_block_reading方法，
	//这个方法可以认为不做任何事情，它的意义在于，目前已经开始处理HTTP请求，除非某个HTTP模块重新设置了read_event_handler方法，
	//否则任何读事件都将得不到处理，也可以认为读事件被阻塞了
    r->read_event_handler = ngx_http_block_reading;
...
ngx_http_run_posted_requests(ngx_connection_t *c)
{
    ...
    for ( ;; ) 
	{
		...
		//如果posted_requests指向NULL空指针，则结束ngx_http_run_posted_requests方法
        if (pr == NULL) 
		{
            return;
        }

		//将主请求的posted_requests指针指向链表中的下一个post请求，当然，下一个post请求有可能不存在，这在下一次循环中就会检测到
		...
		//调用这个post请求ngx_http_request_t结构体中的write_event_handler方法。为什么不是执行read_event_handler方法呢?
...
ngx_http_finalize_request(ngx_http_request_t *r, ngx_int_t rc)
{
	...
	//NGX_DECLINED，表示请求还需要按照11个HTTP阶段继续处理下去
    if (rc == NGX_DECLINED)
	{
		//将content_handler设置为NULL是为了让ngx_http_core_content_phase方法可以继续调用NGX_HTTP_CONTENT_PHASE阶段的其他处理方法
    ...
    {
    	//请求的动作是上传文件，或者HTTP模块需要HTTP框架构造发送并发送响应码大于300以上的特殊响应
		...
		//再次调用ngx_http_finalize_request结束请求
...
//提供给HTTP模块使用的结束请求的方法，但它属于非正常结束的场景，可以理解为强制关闭请求
static void
...
//ngx_http_finalize_connection方法比ngx_http_close_request高了一个层次，
...
ngx_http_finalize_connection(ngx_http_request_t *r)
{
	...
	//count != 1，表示有多个动作在操作着请求
    if (r->main->count != 1)
	{
		//正在丢弃HTTP包体
        if (r->discard_body) 
		{
			//再一次把请求的read_event_handler成员设为ngx_http_discarded_request_body_handler方法
...
ngx_http_set_write_handler(ngx_http_request_t *r)
{
    ...
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
	//在需要限速时，根据计算得到的超时时间已经把写事件添加到定时器中了
    if (!wev->delayed) 
	{
...
ngx_http_writer(ngx_http_request_t *r)
{
    ...
    if (wev->timedout) 
	{
        ...
        if (!wev->delayed) 
		{
			//由于网络异常或客户端长时间不接收响应，导致真实的发送响应超时，结束请求
            ngx_log_error(NGX_LOG_INFO, c->log, NGX_ETIMEDOUT, "client timed out");
            c->timedout = 1;
		...
		//由于上一次发送响应时发送速率过快，超过了请求的limit_rate速率上限，会把delayed标志置为1，设置一个超时时间，
		//将写事件加入定时器，此时是由限速导致超时，并非正真的超时
        wev->timedout = 0;
        wev->delayed = 0;

		//检查写事件的ready标志位，如果为1，则表示在与客户端的TCP连接上可以发送数据，如果为0，则表示暂时不可发送数据
        if (!wev->ready) 
		{
			//不能发送数据
			//将写事件添加到定时器中，这时的超时时间是定时器的就是配置文件中的send_timeout参数
            ngx_add_timer(wev, clcf->send_timeout);
			//将写事件添加到epoll事件循环中
            if (ngx_handle_write_event(wev, clcf->send_lowat) != NGX_OK)
			{
	...
	//调用方法发送响应，其中第2个参数(也就是表示需要发送的缓冲区)为NULL指针。
	...
	//将请求的write_event_handler方法设置为ngx_http_request_empty_handler，也就是说，
	//如果这个请求的连接上再有可写事件，将不做任何处理
    r->write_event_handler = ngx_http_request_empty_handler;
...
ngx_http_block_reading(ngx_http_request_t *r)
{
	...
	//这个函数实际上什么都不做
	//在事件模型设置为水平触发时，唯一做的事情就是将事件从事件模型监听列表中删除，防止该事件一直被触发
...
//将主请求的引用计数减1，在引用计数清零时正式调用ngx_http_free_request方法和ngx_http_close_connection方法来释放请求、关闭连接
static void
ngx_http_close_request(ngx_http_request_t *r, ngx_int_t rc)
{
	...
	//检查count引用计数是否已经为0，以及blocked标志位是否为0。
	//如果count为0，则证明请求没有其他动作要使用了，如果blocked为0，表示没有http模块还需要处理请求

	//注意，在官方发布的HTTP模块中，ngx_http_request_t结构体中的blocked标志位主要由异步I/O使用，
	//ngx_http_close_request方法正是通过配合着异步I/O工作，如果AIO上下文中还在处理这个请求，blocked必然是大于0的，
	//这时ngx_http_close_request方法不能结束请求
...
ngx_http_free_request(ngx_http_request_t *r, ngx_int_t rc)
{
	...
	//循环的遍历请求结构体中的cleanup链表，依次调用每一个ngx_http_cleanup_pt方法释放资源
    cln = r->cleanup;
    r->cleanup = NULL;
	...
	//依次调用NGX_HTTP_LOG_PHASE阶段的所有回调方法记录日志
    ngx_http_log_request(r);
	...
	//销毁请求结构体中的pool内存池。在销毁内存池时，挂在该内存池下的由各Nginx模块实现的ngx_pool_cleanup_t方法也会被调用
    pool = r->pool;
    r->pool = NULL;
