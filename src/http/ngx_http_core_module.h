...
typedef struct 
{
	...
	//存储变量名的散列表， 调用ngx_http_get_variable方法获取未索引的变量值时就靠这个散列表找到变量的解析方法
	//以hash表的形式存储nginx模块内部定义提供给外部使用的变量(除了以arg_,http_,sent_http_,cookie_,upstream_http_开头的变量和显示设置不要hash的变量)
    ngx_hash_t                 variables_hash;
	// 存储索引过的变量的数组， 通常各模块使用变量时都会在Nginx启动阶段从该数组中获得索引号，
	// 这样， 在Nginx运行期内， 如果变量值没有被缓存， 就会通过索引号在variables数组中找到变量的定义， 再解析出变量值
	///以数组的形式存储用户在配置文件中使用的变量(通过ngx_http_get_variable_index()函数而添加到variables中)
	///方便使用该变量的模块(通过配置文件)，通过以索引的方式获取该变量
	...
	//临时存储nginx模块内部定义提供给外部使用的变量
	//用于检查用户在配置文件中使用的变量是否都已经被定义
