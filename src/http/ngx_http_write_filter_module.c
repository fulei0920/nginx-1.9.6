...
//µ±Ò»´ÎÎÞ·¨·¢ËÍÈ«²¿µÄ»º³åÇøÄÚÈÝÊ±£¬ngx_http_write_filter·½·¨ÊÇ»á·µ»ØNGX_AGAINµÄ(Í¬Ê±½«Î´·¢ËÍÍê³ÉµÄ»º³åÇø·Åµ½ÇëÇóµÄout³ÉÔ±ÖÐ)
//²ÎÊý
// in -- ±¾´ÎÒª·¢ËÍµÄ»º³åÇøÁ´±í
ngx_int_t
ngx_http_write_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    for (cl = r->out; cl; cl = cl->next) 
	{
        ...
        ngx_log_debug7(NGX_LOG_DEBUG_EVENT, c->log, 0, "write old buf t:%d f:%d %p, pos %p, size: %z file: %O, size: %O",
        ...
        if (ngx_buf_size(cl->buf) == 0 && !ngx_buf_special(cl->buf)) 
		{
            ngx_log_error(NGX_LOG_ALERT, c->log, 0, "zero size buf in writer t:%d r:%d f:%d %p %p-%p %p %O-%O",
                          cl->buf->temporary, cl->buf->recycled, cl->buf->in_file, cl->buf->start, cl->buf->pos, cl->buf->last,
                          cl->buf->file, cl->buf->file_pos, cl->buf->file_last);
    ...
    *ll = NULL;
	...
	//Èç¹ûdelayedÎª1£¬Ôò±íÊ¾ÕâÒ»´ÎµÄepollµ÷¶ÈÖÐÇëÇóÈÔÐèÒª¼õËÙ£¬ÊÇ²»¿ÉÒÔ·¢ËÍÏìÓ¦µÄ
    if (c->write->delayed)
	{
		//¸æËßHTTP¿ò¼Üout»º³åÇøÖÐ»¹ÓÐÏìÓ¦µÈ´ý·¢ËÍ
    ...
    if (r->limit_rate) 
	{
		...
		//¼ÆËã±¾´Î¿ÉÒÔ·¢ËÍµÄ×î´ó×Ö½ÚÊý
        limit = (off_t) r->limit_rate * (ngx_time() - r->start_sec + 1) - (c->sent - r->limit_rate_after);

        if (limit <= 0)
		{
			//·¢ËÍÏìÓ¦µÄËÙÂÊ´ïµ½ÉÏÏÞ....
			
            c->write->delayed = 1;
			//¼ÆËã³¬Ê±Ê±¼ä£¬½«Ð´ÊÂ¼þ¼ÓÈëµ½¶¨Ê±Æ÷ÖÐ£¬Ê¹Nginx¶¨Ê±Æ÷×¼È·µØÔÚÔÊÐí·¢ËÍÏìÓ¦Ê±¼¤»îÇëÇó£¬
			//Õâ¸ö¶¨Ê±Æ÷µÄ³¬Ê±Ê±¼äÊÇ³¬·¢×Ö½ÚÊý°´ÕÕlimit_rateËÙÂÊËã³öÐèÒªµÈ´ýµÄÊÂ¼þÔÙ¼ÓÉÏ1ºÁÃë£¬
			//limitÊÇÒÑ¾­³¬·¢µÄ×Ö½ÚÊý£¬ËüÊÇ0»òÕß¸ºÊý£
	...
	//ÖØÖÃ½á¹¹ÌåµÄ»º³åÇø£¬°ÑÒÑ¾­·¢ËÍ³É¹¦µÄ»º³åÇø¹é»¹¸øÄÚ´æ³Ø
    ...
    if (chain) 
	{
		//outÁ´±íÖÐ»¹ÓÐÊ£ÓàµÄÃ»ÓÐ·¢ËÍ³öÈ¥µÄ»º³åÇø
        c->buffered |= NGX_HTTP_WRITE_BUFFERED;
        return NGX_AGAIN;
    }

	//outÁ´±íÖÐËùÓÐµÄ»º³åÇø¶¼·¢ËÍ¸ø¿Í»§¶ËÁË  ???
    c->buffered &= ~NGX_HTTP_WRITE_BUFFERED;
